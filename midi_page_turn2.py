...
def midi_page_turn():
    ...
    for i in range(pygame.midi.get_count()):
        ...
        print(
            '[{0:d}]   {1:7s} {2:10s} {3:^50s}'.format(i, typestr, ("OK" if ret[4] else "NA"), ret[1].decode("utf-8")))
    ...
    try:
            ...
            for d in data:
                ...
                if val > 0:
                    if ccdata[cc][0]:
                        ...
                        # spinner.stop()
                        spinner.write(
                            '  🎼 {0} {1:d}'.format(('NEXT' if cc == LEFT_PEDAL else "PREV"), int(time.time())))
                    # spinner.start()
                    # spinner.ok('✔')
                    # print('-'*20)
        ...
        # initialized = True
        # if version.parse(pygame.__version__) >= version.parse('1.9.5'):
        #     initialized = pygame.midi.get_init()
...
if not is_windows():
    print("Please select musicsheet viewer window")
